...
export default function HomeClient({ initialVideoId }: HomeClientProps) {
  ...
  const filteredNews = useMemo(() => {
    ...
    return baseFilteredNews.filter((item) => !watchedMap[item.youtubeId]);
  ...
  useEffect(() => {
    ...
    if (items.some((item) => item.id === initialVideoId)) {
      setActiveId(initialVideoId);
    }
    initialHandledRef.current = true;
  }, [initialVideoId, items, isLoading]);
  ...
  }, [queueItems, activeId, queueMode, updateQuery]);
